...
export function getTranslationDirectionOptions() {
  const options = [
    { value: 'intelligent', label: '🧠 Intelligent multilingual mode' },
    { value: 'vi-to-en', label: 'Dịch từ Việt sang Anh' },
    { value: 'en-to-vi', label: 'Dịch từ Anh sang Việt' },
  ];

  // 添加其他常用语言的传统翻译选项
  const popularLanguages = ['ja', 'ko', 'fr', 'de', 'es', 'ru', 'vi'];
  for (const langCode of popularLanguages) {
    const language = LANGUAGES[langCode];
    if (language) {
      options.push({
        value: `vi-to-${langCode}`,
        label: `Dịch từ Việt sang ${language.nativeName}`,
      });
    }
  }
